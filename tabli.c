/* tabli v1 — a self-contained table: this executable carries its own data.
 *
...
#define ENGINE_VERSION "tabli v1"
#define FMT_VERSION 1
#define HDR_SIZE 64
#define MAGIC "TABLI001" /* 8 bytes */
...
static void tab_load(Tab *t) {
    ...
    if (esz < 0 || esz > st.st_size)
        die(5, "# error: %s is not a tabli file (bad ELF layout)", t->path);
    t->engine_size = esz;
    ...
    if (xpread(fd, hb, HDR_SIZE, esz) != HDR_SIZE || hdr_unpack(hb, &t->h) != 0)
        die(5, "# error: %s: no tabli header found — not a table, or corrupt", t->path);
