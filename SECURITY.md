...
tabli's design is unusual — data files are executables — so its trust model is
...
A `.tbl` file found on disk may be a genuine tabli table or a binary that
merely looks like one. No amount of in-engine verification can protect you if
you execute a malicious file: the decision happens before tabli's code runs.
