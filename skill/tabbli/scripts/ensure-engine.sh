#!/bin/sh
# Prints the path to a tabbli engine for this machine's architecture,
...
ARCH=$(uname -m)
BIN="$DIR/bin/tabbli-$ARCH"
if [ ! -x "$BIN" ]; then
  command -v cc >/dev/null 2>&1 || {
    echo "# error: no prebuilt engine for $ARCH and no C compiler (cc) found" >&2
    echo "# install gcc/clang, or copy a tabbli-$ARCH binary into $DIR/bin/" >&2
    exit 1
  }
  mkdir -p "$DIR/bin"
  cc -O2 -Wall -Wextra -D_FORTIFY_SOURCE=2 -fstack-protector-strong -fPIE \
     "$DIR/src/tabbli.c" -o "$BIN" -pie -Wl,-z,relro,-z,now
  strip "$BIN" 2>/dev/null || true
  echo "# built tabbli engine for $ARCH from bundled source" >&2
fi
echo "$BIN"
